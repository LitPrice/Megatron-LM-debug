...
__all__ = ['RotaryEmbedding', 'RotaryEmbeddingOld', 'apply_rotary_pos_emb']
...
class RotaryEmbedding(torch.nn.Module):
    ...
    def forward(self, seq_len):
        ...
        if self.max_seq_len_cached is None or (seq_len > self.max_seq_len_cached):
            ...
            t = torch.arange(self.max_seq_len_cached, device=torch.cuda.current_device(), dtype=torch.float32)
            t = t / self.scale_factor
            freqs = torch.einsum('i,j->ij', t, self.inv_freq)
            # Different from paper, but it uses a different permutation in order to obtain the same calculation
            emb = torch.cat((freqs, freqs), dim=-1).to(torch.cuda.current_device())
